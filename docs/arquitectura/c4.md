...
C4Context
    ...
    System_Ext(llm, "Proveedor LLM", "Claude u OpenAI, detrás del puerto LLMProvider")
...
| Proveedor LLM | Sistema externo | Claude u OpenAI, elegido por configuración, detrás del puerto `LLMProvider` | SPEC-05 · RNF *Configuración*; [ADR-0005](adr/ADR-0005-llm-con-tool-calling-detras-de-llmprovider.md) |
...
C4Container
    ...
    Rel(api, llm, "Tool calling con streaming", "HTTPS")
...
| Worker de tareas | Contenedor | Python, APScheduler | `OutboxWorker` (tareas `NOTIFICAR_PAGO_VENTAS`, `SOLICITAR_ANULACION`, `ENVIAR_CORREO`, con backoff exponencial) y jobs programados: `ExpirarCheckouts` (cada minuto), limpieza de carritos anónimos (7 días) y de referencias de evidencia de borradores (24 h) | README §1.5; `grabacion-pedido/design.md`, `checkout-pago/design.md`, `gestion-carrito/design.md`, `solicitud-devolucion-cambio/design.md`; [ADR-0011](adr/ADR-0011-outbox-transaccional.md) |
| Base de datos del chatbot | Contenedor (datos) | PostgreSQL | Tablas propias: `conversacion`, `mensaje`, `carrito`, `item_carrito`, `checkout`, `intento_pago`, `celular_verificacion_local`, `pedido_ref`, `notificacion`, `reclamo_ref`, `devolucion_ref`, `evidencia`, `outbox` | [`modelo-datos.md`](../modelo-datos.md) |
...
| Backend API → Proveedor LLM | HTTPS (API del proveedor) | Tool calling con streaming; timeout de 15 s |
...
C4Component
    ...
    Container_Boundary(api, "Backend API (FastAPI)") {
        ...
        Component(llmp, "LLMProvider", "Puerto outbound", "ClaudeProvider u OpenAIProvider")
    }

    ContainerDb(db, "PostgreSQL", "PostgreSQL", "conversacion, mensaje")
...
|---|---|---|---|
| `chatbot_router.py` | Adaptador inbound (REST) | `POST/GET /chat/conversaciones`, `GET /chat/conversaciones/buscar`, `GET/POST /chat/conversaciones/{id}/mensajes`. Aplica `RateLimiter` (20 mensajes/min por cliente o IP) y `JwtValidator` (`get_cliente_actual`). Invoca `ChatbotServicePort` | `motor-conversacion/design.md`; SPEC-05 · Req. 1, 4, 11 |
...
| `LLMProvider` + `ClaudeProvider` / `OpenAIProvider` | Puerto outbound + adaptadores | Llamada con tools, streaming y timeouts; proveedor, modelo, temperatura y `max_tokens` por variables de entorno | SPEC-05 · RNF *Configuración* |
| `conversacion_postgres_adapter` | Adaptador outbound | Persistencia de `conversacion` y `mensaje` | README §1.2; [`modelo-datos.md`](../modelo-datos.md) |
