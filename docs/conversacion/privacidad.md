...
| Specs del canal | SPEC-05 · Req. 9, 12 y RNF *Privacidad*; SPEC-23 (RNF de privacidad de las imágenes); SPEC-01, 03, 04, 12, 14, 16, 18, 19, 21 (RNF de privacidad y seguridad) |
...
| **Imágenes que el cliente adjuntó al chat** (base64, sin EXIF ni ubicación, últimos 12 mensajes, máx. 6 por turno; SPEC-23) | Evidencia de devolución (SPEC-21); nombres originales de los archivos; URLs de las imágenes |
| | URL de la evidencia (SPEC-21 · RNF); las imágenes del chat nunca se envían como URL |
...
3. ⚠️ **Riesgo residual de las imágenes (SPEC-23).** `SensitiveDataFilter` solo actúa sobre texto: una imagen puede mostrar rostros, un documento de identidad, una tarjeta o una dirección y llegar al LLM sin redacción automática. Se mitiga con el aviso previo a la primera carga ("Evita fotos con rostros, documentos, tarjetas o datos personales"), la eliminación de EXIF y ubicación, la instrucción al asistente de no transcribir esos datos y de recordar que los datos de tarjeta y documento se ingresan solo en la pantalla de pago. **Riesgo aceptado** en esta versión; una detección o un aviso adicional queda como pregunta abierta de SPEC-23.

**Retención por el proveedor.** OpenAI conserva las entradas de la API (incluidas las imágenes) durante 30 días para monitoreo de abuso, según sus términos vigentes (**a verificar** antes de operar). Ese plazo no reemplaza una política propia y no depende de la retención local de los adjuntos. El proveedor no actúa como almacenamiento: por eso las imágenes se guardan en el bucket privado del chatbot.
...
- Informarlo en la política de privacidad (destinatario, país, finalidad), incluido que las imágenes adjuntas al chat se envían al proveedor y que este las conserva 30 días para monitoreo de abuso.
...
### Aviso antes de la primera imagen
Definido en SPEC-23 · Req. 2: antes de la primera carga de una imagen en el dispositivo se muestra "Las imágenes que adjuntes se envían a un asistente con IA y se guardan de forma privada. Evita fotos con rostros, documentos, tarjetas o datos personales." con el enlace "Política de privacidad" y el botón "Entendido, adjuntar", sin casillas premarcadas. La confirmación se recuerda por dispositivo (LocalStorage) y no bloquea el envío de texto.
...
| Destinatarios | Módulos del Marketplace: Seguridad y Usuarios, Productos y Ofertas, Ventas y Postventa, Despacho y Entrega; el proveedor LLM (que también recibe las imágenes adjuntas al chat); el proveedor de correo (SMTP); el proveedor de almacenamiento de imágenes (Supabase Storage) |
| Transferencia internacional | Proveedor LLM (país y finalidad), con las imágenes adjuntas y su conservación de 30 días por el proveedor |
...
| Almacenamiento en el navegador | Token en LocalStorage, cookies `chat_sid` y de refresh, y la confirmación del aviso de imágenes en LocalStorage |
...
|---|---|---|
| LLM01 Prompt Injection | Resultados de herramientas delimitados como datos; texto dentro de una imagen tratado como dato; descuentos solo de Productos | ✅ SPEC-05 · Req. 9; SPEC-23 · Req. 5 |
| LLM02 Sensitive Information Disclosure | Redacción de tarjeta, OTP y contraseña en el texto; nombre de pila como único dato; lista blanca de Despacho; las imágenes no se redactan (aviso previo y EXIF eliminado) | ✅ SPEC-05 · Req. 9 y RNF; SPEC-18 · RNF · ⚠️ brechas del §3 (incluido el riesgo residual de las imágenes, SPEC-23) |
...
| LLM10 Unbounded Consumption | 20 mensajes/min, máx. 5 iteraciones de herramientas, `max_tokens`, timeout de 15 s; 10 cargas de imagen/min y máx. 6 imágenes por turno | ✅ SPEC-05 · Req. 11, RNF y `design.md`; SPEC-23 · Req. 9 y RNF |
